...
def create_features(history, window=7):
    ...
    for i in range(window, len(hist_num)):
        ...
        features = ws + [ent, streak, ratio_tai, wma, pattern_consistency, ac1, ac2, alt,
                         bias['var'], runs_z, spec_bias, anom_score, binom_p, binom_dev]
        X.append(features)
        y.append(hist_num[i])
    if not X:
...
def expert_catboost_prob(catboost_model, history, window=7):
    if catboost_model is None or len(history) < window:
        return 0.5
    X_all, _ = create_features(history, window)
    if X_all.size == 0:
        return 0.5
    return catboost_model.predict_proba([X_all[-1]])[0][1]
...
class RLPolicy:
    def __init__(self, state_size=12, n_experts=9):
...
def combined_predict(session_state, history, window=7, label_smoothing_alpha=0.1, risk_threshold=0.55, skip_on_high_risk=True):
    ...
    default_result = {
        ...
        "expert_probs": [0.5] * 9,
        "weights": s["meta"].get("weights", np.ones(9) / 9.0),
...
if "catboost_model" not in st.session_state: st.session_state.catboost_model = None
...
if recent:
    ...
    alt = alternation_score(recent)
    st.write(f"Entropy (base2): {pred_info['entropy']:.3f} | Autocorr1: {ac1:.3f} | Autocorr2: {ac2:.3f} | Alternation: {alt:.3f}")
...
if len(st.session_state.history) >= 2:
    ...
    if len(history_before) >= window:
        ...
        state = [ent_val, streak, pred_info['risk_score'], bias_level, np.mean(losses), ac1, ac2, alt]
        ...
        if len(history_before) > window and len(st.session_state.history) % 5 == 0:
            ...
            if Xb.size > 0:
                ...
                if st.session_state.catboost_model is None:
                    st.session_state.catboost_model = CatBoostClassifier(iterations=50, random_state=42, silent=True)
                st.session_state.catboost_model.fit(new_X, new_y)
